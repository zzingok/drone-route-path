...
public class ObliquePhotography5RouteAlgorithm {
    ...
    private static class SpatialGrid {
        ...
        public List<LatLngPoint> getNearbyPoints(LatLngPoint point, double radius) {
            List<LatLngPoint> nearby = new ArrayList<>();
            int cellRadius = (int) Math.ceil(radius / cellSize);

            String centerKey = getCellKey(point);
            ...
            for (int dx = -cellRadius; dx <= cellRadius; dx++) {
                for (int dy = -cellRadius; dy <= cellRadius; dy++) {
                    ...
                    List<LatLngPoint> cellPoints = grid.get(key);
                    if (cellPoints != null) {
                        for (LatLngPoint cellPoint : cellPoints) {
                            if (calculateDistanceFast(point, cellPoint) <= radius) {
                                nearby.add(cellPoint);
                            }
                        }
                    }
                }
    ...
    private static double fastSin(double degrees) {
        int index = (int) Math.round(degrees) % 360;
        if (index < 0) {
            index += 360;
        }
    ...
    private static LatLngPoint borrowPoint() {
        LatLngPoint point = POINT_POOL.get().poll();
        if (point == null) {
            point = new LatLngPoint();
        }
        return point;
